...
public class Collections3 {
    ...
    public static List<String> splitToList(String value, String separator, boolean filter) {
        Iterator<String> iterator = null;
        Splitter splitter = Splitter.on(separator).trimResults();
        if (filter) {
            iterator = splitter.omitEmptyStrings().split(value).iterator();
        } else {
            iterator = splitter.split(value).iterator();
        }
        List<String> result = Lists.newArrayList(iterator);
        return result;
    }

    public static String joinSkipNulls(Object[] objects) {

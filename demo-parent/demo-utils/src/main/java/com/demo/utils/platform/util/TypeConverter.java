...
public class TypeConverter {
	...
	private static final int TIMESTAMP_LENGTH = TIMESTAMP_PATTERN.length();
	...
	public static final Object convert(Class<?> type, String s) throws ParseException {
		...
		if (type == String.class) {
			...
			// zheng.shk  update by 2016/11/15
			return s;
		}
		s = s.trim();
		// 前面的 String跳过以后,所有的空字符串全都转成 null,  这是合理的
		if ("".equals(s)) {
			return null;
		}
		...
		if (type == java.util.Date.class) {
			// if (x < TIMESTAMP_LENGTH) 改用 DATE_PATTERN 转换，更智能
			if (s.length() >= TIMESTAMP_LENGTH) {
				...
				return new SimpleDateFormat(TIMESTAMP_PATTERN).parse(s);
			}
			else {
		...
		if (type == java.sql.Date.class) {
			...
			if (s.length() >= TIMESTAMP_LENGTH) {
				...
				return new java.sql.Date(new SimpleDateFormat(TIMESTAMP_PATTERN).parse(s).getTime());
			}
			else {
				// return new java.sql.Date(java.sql.Date.valueOf(s).getTime());	// error under jdk 64bit
				return new java.sql.Date(new SimpleDateFormat(DATE_PATTERN).parse(s).getTime());
			}
		}
		...
		if (type == java.sql.Timestamp.class) {
			if (s.length() >= TIMESTAMP_LENGTH) {
				...
				return new java.sql.Timestamp(new SimpleDateFormat(DATE_PATTERN).parse(s).getTime());
			}
		}
